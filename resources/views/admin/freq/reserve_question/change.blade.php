...
@section('title', $reserve_type_title.'予約設定')

@section('content')
    <div id="contents">
        @include('admin.layouts.header_under', ['action_url'=>route('admin.freq.reserve_question.edit', ['question'=>$question->que_id, 'reserve_type'=>$reserve_type])])

        {{ Form::open(["route"=>"admin.freq.reserve_question.changeDate", "method"=>"post", "name"=>"frm_question", "id"=>"frm_question"]) }}
            ...
            <section>
                <div id="main-contents">
                    <div class="search-result-area">
                                ...
                                <td><div class="select-box">
                                        <select class="text-left" name="year" id="year" onchange="setLeapMonth('')">
                                            ...
                                            @foreach(range(\Carbon\Carbon::now()->format('Y') + 5, \Carbon\Carbon::now()->format('Y')-5) as $year)
                                                <option value="{{ $year }}" {{ $year == old('year', \Carbon\Carbon::parse($reserve_date_at)->format('Y')) ? 'selected' : '' }}>{{ $year }}</option>
                                            @endforeach
                                        </select>
                                    </div></td>
                                ...
                                <td><div class="select-box">
                                        <select class="text-left" name="month" id="month" onchange="setLeapMonth('birthday')">
                                            ...
                                            @foreach(range(1, 12) as $month)
                                                <option value="{{ str_pad($month, 2, "0", STR_PAD_LEFT) }}" {{ $month == old('month', \Carbon\Carbon::parse($reserve_date_at)->format('m')) ? 'selected' : '' }}>{{ $month }}</option>
                                            @endforeach
                                        </select>
                                    </div></td>
                                ...
                                <td><div class="select-box">
                                        <select class="text-left" name="day" id="day">
                                            ...
                                            @foreach(range(1, 31) as $day)
                                                <option value="{{ str_pad($day, 2, "0", STR_PAD_LEFT) }}" {{ $day == old('day', \Carbon\Carbon::parse($reserve_date_at)->format('d')) ? 'selected' : '' }}>{{ $day }}</option>
                                            @endforeach
                                        </select>
                                    </div></td>
                            ...
                            <tr>
                                <td><div class="select-box">
                                        <select name="hour" class="text-left" id="hour">
                                            @for ($i = 0; $i < 24; $i++)
                                                <option value="{{$i}}"
                                                        @if(old('hour', \Carbon\Carbon::parse($reserve_date_at)->format('H')) == $i) selected="selected" @endif>{{ str_pad($i, 2, "0", STR_PAD_LEFT) }}</option>
                                            @endfor
                                        </select>
                                    </div></td>
                                ...
                                <td><div class="select-box">
                                        <select name="minute" class="fourth" id="minute">
                                            @for ($i = 0; $i < 60; $i++)
                                                <option value="{{$i}}"
                                                        @if(old('minute', \Carbon\Carbon::parse($reserve_date_at)->format('i')) == $i) selected="selected" @endif>{{ str_pad($i, 2, "0", STR_PAD_LEFT) }}</option>
                                            @endfor
                                        </select>
                                    </div></td>
                        ...
                        @enderror
                        <p id="warning_date_validate" class="error_text hide">{{ $reserve_type_title }}予約日時を現在時刻以降に設定してください。</p>
                        <p id="warning_reserve_date" class="error_text hide">{{ $reserve_type_title }}予約日時を正確に入力してください。</p>
                        <div class="btn mtb">
                            <button type="submit" class="finish modal-syncer" data-target="" id="btn_reserve">変更を確定する</button>
                        </div>
                        <div class="btn mtb">
                            <button type="button" onclick="location.href='{{ route('admin.freq.reserve_question.edit', ['question'=>$question->que_id, 'reserve_type'=>$reserve_type]) }}'">戻る</button>
                        </div>
                    </div>
                </div>
            </section>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#year, #month, #day, #hour, #minute').change(function() {
                ...
                let msg = '<strong>{{ $reserve_type_title }}日時</strong><br><br>' + year + '年' + month + '月' + day + '日' + hour + '時' + minute + '分';
