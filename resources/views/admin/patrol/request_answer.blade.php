...
@section('content')
    <div id="contents">
        ...
        {{ Form::open(["route"=>"admin.patrol.request_answer", "method"=>"get", "name"=>"frm_history", "id"=>"frm_history"]) }}
            ...
            <section class="tab_area mb0">
                <div class="switch_tab">
                    <div class="type_radio radio-01">
                        <input type="radio" name="onof-line" id="off-line" onclick="location.href='{{ route('admin.patrol.request_send') }}'">
                        <label class="ok" for="off-line">リクエスト送信</label>
                    </div>
                    <div class="type_radio radio-02">
                        <input type="radio" name="onof-line" id="on-line-1" checked="checked" onclick="location.href='{{ route('admin.patrol.request_answer') }}'">
                            ...
                            <div class="form_wrap icon_form type_arrow_bottom">
                                <select name="search_params[status]" id="status">
                                    ...
                                    @foreach(config('const.lesson_request_status') as $key=>$value)
                                        @if($key != config('const.lesson_request_status_code.reserve_request') && $key != config('const.lesson_request_status_code.attendance_request'))
                ...
                @if(count($obj_lesson_requests) > 0)
                    @foreach($obj_lesson_requests as $lesson_request)
                        ...
                        <div class="lesson_box admin_lesson_box">
                            <a href="{{ route('admin.patrol.request_answer_detail', ['lessonRequest'=>$lesson_request->lr_id]) }}">
                                <div class="flex flex-wrap flex-label">
                                    <div class="status-type">{{ $lesson_request->status_name }}</div>
                                    <div>{{ $lesson_request->lr_response_date ? \Carbon\Carbon::parse($lesson_request->lr_request_date)->format('Y-m-d H:i') : '' }}</div>
                                </div>
