...
@section('content')
    <div id="contents">
        ...
        <div class="tabs form_page">
            <label class="page-title" style="width: 100%;">レッスン履歴</label>

            <section class="tab_area mb0">
                <div class="switch_tab">
                    <div class="type_radio radio-01">
                        <input type="radio" name="onof-line" id="off-line" checked="checked" onclick="location.href='{{ route('admin.lesson_history_management.lesson') }}'">
                        <label class="ok" for="off-line">出品レッスン</label>
                    </div>
                    <div class="type_radio radio-02">
                        <input type="radio" name="onof-line" id="on-line-1" onclick="location.href='{{ route('admin.lesson_history_management.recruit') }}'">
                        ...
                        <div class="inner_box for-warning">
                            ...
                            <div class="form_wrap icon_form type_arrow_bottom">
                                <select name="search_params[status]" id="status">
                                    <option value="">--</option>
                                    @foreach(config('const.lesson_history_status') as $key=>$value)
            ...
            <section>
                ...
                <div class="flex" style="align-items: center;margin-bottom: 10px;">
                    @php
                        $from_page = ($obj_lesson_request_schedules->currentPage() - 1) * $obj_lesson_request_schedules->perPage() + 1;
                        $to_page = $obj_lesson_request_schedules->perPage() * $obj_lesson_request_schedules->currentPage();
                        if ($to_page > $obj_lesson_request_schedules->total()) {
                            $to_page = $obj_lesson_request_schedules->total();
                        }
                    @endphp
                    @if($obj_lesson_request_schedules->total() <= 1)
                        <div class="wp-50">全{{ $obj_lesson_request_schedules->total() }}件</div>
                    @else
                        <div class="wp-50">{{ $from_page }}件～{{ $to_page }}件（全{{ $obj_lesson_request_schedules->total() }}件）</div>
                    @endif
                ...
                @if(count($obj_lesson_request_schedules) > 0)
                    @foreach($obj_lesson_request_schedules as $lesson_request_schedule)
                        @php
                            $lesson_request = $lesson_request_schedule->lesson_request;
                            $lesson = $lesson_request->lesson;
                            $lesson_senpai = $lesson_request_schedule->senpai;
                            $lesson_kouhai = $lesson_request_schedule->kouhai;
                        @endphp
                        <div class="lesson_box admin_lesson_box">
                            <a href="{{ route('admin.lesson_history_management.lesson_detail', ['lesson_request_schedule'=>$lesson_request_schedule->lrs_id]) }}">
                                <div class="flex flex-wrap flex-label">
                                    <div class="status-type">{{ $lesson_request_schedule->status_name }}</div>
                                    <div>{{ $lesson_request_schedule->history_updated_at }}</div>
                                </div>
                                <div class="flex flex-wrap flex-center">
                                    {{--$lesson_senpai->user_sex--}}
                                    <div class="{{ $lesson_request_schedule->senpai->user_sex == config('const.sex.woman') ? 'pink-color' : 'water-color' }}">{{ $lesson_senpai->name }}<span>（{{\App\Service\CommonService::getAge($lesson_senpai->user_birthday)}}）</span>先輩</div>
                                    <div>～</div>
                                    <div class="water-color">{{ $lesson_kouhai->name }}<span>（{{\App\Service\CommonService::getAge($lesson_kouhai->user_birthday)}}）</span>後輩</div>
                                </div>
                                </p>
                            </a>
                        </div>
                ...
                {{ $obj_lesson_request_schedules->links('vendor.pagination.senpai-admin-pagination') }}
            </section>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#sort_type').change(function() {
                ...
                location.href="{{ route('admin.lesson_history_management.lesson') }}?order="+order_type;
            });
