...
@section('content')
    <div id="contents">
        ...
        {{ Form::open(["route"=>"admin.fraud_report.index", "method"=>"get", "name"=>"form1", "id"=>"form1"]) }}
        <input type="hidden" name="clear_condition" id="clear_condition" value="">
        <div class="tabs form_page">
            <label class="page-title" style="width: 100%;">通報者一覧</label>
            <section>
            ...
            <section>
                <div class="chk-rect mb-10">
                    <input type="checkbox" value="1" name="search_params[chk-not-read]" id="chk-not-read" {{ isset($search_params['chk_not_read']) && $search_params['chk_not_read'] ? 'checked' : '' }}>
                ...
                <div class="flex" style="align-items: center;margin-bottom: 10px;">
                    ...
                    <div class="wp-50 form_wrap icon_form type_arrow_bottom">
                        <select class="wp-100" name="search_params[sort_type]" id="sort_type">
                            <option value="">--</option>
                            @foreach(config('const.report_sort') as $key=>$val)
                                <option value="{{ $key }}" {{ (isset($search_params['order']) && $search_params['order'] ? $search_params['order'] : config('const.report_sort_code.register_new')) == $key ? 'selected' : '' }}>{{ $val }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <table>
                        <tbody>
                            @if(count($obj_users) > 0)
                                @foreach($obj_users as $obj_appeal_user)
                                    @php
                                        $obj_user = $obj_appeal_user->appeal_user;
                                    @endphp
                                    <tr class="{{ $obj_appeal_user->exist_not_read == 0 ? 'unread' : '' }}">
                                        <td class="td-detail"><a class="title_orange" href="{{ route('admin.fraud_report.detail', ['user'=>$obj_user->id]) }}">詳細</a></td>
                                        <td>
                                            <div class="flex profile" data-id="{{ $obj_user->id }}">
                                                <div class="ico ico-user">
                                                    <img src="{{ \App\Service\CommonService::getUserAvatarUrl($obj_user->user_avatar) }}">
                                                </div>
                                                <div>
                                                    <p class="pb-5 ft-bold">{{ $obj_user->user_firstname.'　'.$obj_user->user_lastname }}<span>{{ "（".\App\Service\CommonService::getAge($obj_user->user_birthday)."）" }}</span></p>
                                                    <p class="pb-5">{{ $obj_user->user_sex ? config('const.gender_type.'.$obj_user->user_sex) : '' }}&nbsp;{{ \App\Service\AreaService::getOneAreaFullName($obj_user->user_area_id) }}</p>
                                                    {{--<p class="">最終更新日：{{ \Carbon\Carbon::parse($obj_appeal_user->last_updated_at)->format('Y.n.j H:i:s') }}</p>--}}
                                                    <p class="">最終更新日：{{ \Carbon\Carbon::parse($obj_appeal_user->end_reported_at)->format('Y.n.j H:i:s') }}</p>
                                                </div>
                                            </div>
                                        </td>
                                        {{--<td class="vertical-am"><div class="flex-space cnt_count">{{ \App\Service\AppealService::getAppealCountByCondition(['user_id'=>$obj_user->id, 'chk_not_read'=>(isset($search_params['chk_not_read']) && $search_params['chk_not_read'] ? 1 : '' )]) }}件</div></td>--}}
                                        <td class="vertical-am"><div class="flex-space cnt_count">{{ $obj_appeal_user->cnt_appeal }}件</div></td>
                                    </tr>
                                @endforeach
                            @else
                                <tr>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.profile').click(function() {
                ...
                location.href = "{{ route('admin.fraud_report.detail') }}" + "/" + user_id;
            });
            ...
            $('#chk-not-read').change(function() {
                ...
                location.href="{{ route('admin.fraud_report.index') }}?chk_not_read="+chk_not_read;
            });

            $('#sort_type').change(function() {
                let order_type = $(this).val();
                location.href="{{ route('admin.fraud_report.index') }}?order="+order_type;
            });
        });
    </script>
@endsection
