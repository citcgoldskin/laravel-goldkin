...
@section('content')
    <div id="contents">
        {{ Form::open(["route" => "admin.fraud_stop_recruit.do_cancel", "method"=>"post", "name"=>"form1", "id"=>"form1"]) }}
            <input type="hidden" name="recruit_id" value="{{ $recruit_id }}">
            <section class="pt20">
                <div class="lesson_info_area">
                    <label class="page-title" style="width: 100%;">この投稿の公開停止を取り消します。</label>
        ...
        {{ Form::close() }}
            ...
            <section id="info_area">
                ...
                <button class="btn wp-100 mb-10 mt-20" onclick="location.href='{{ route('admin.fraud_stop_recruit.detail') }}/{{$recruit_id}}'">戻る</button>
            </section>
        ...
        $(document).ready(function() {
            ...
            $('.chk-cancel-stop-reserve').change( function() {
                if($('[id=chk-cancel-stop-reserve]').prop('checked')){
                    ...
                    let cancel_hour = $('#cancel_hour').val('');
                    let cancel_minute = $('#cancel_minute').val('');
                    let msg = '当該投稿を' + cancel_date + '日' + cancel_hour + '時' + cancel_minute + '分に取り消す';
