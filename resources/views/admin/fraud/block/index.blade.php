...
@section('content')
    <div id="contents">
        ...
        {{ Form::open(["route"=>"admin.fraud_block.index", "method"=>"get", "name"=>"form1", "id"=>"form1"]) }}
        <input type="hidden" name="clear_condition" id="clear_condition" value="">
        <div class="tabs form_page">
            <label class="page-title" style="width: 100%;">ブロック一覧</label>
            <section>
                ...
                <div class="tabs search-result-area">
                    <table>
                        ...
                        @if(count($obj_users) > 0)
                            @foreach($obj_users as $obj_block_user)
                                @php
                                    $obj_user = $obj_block_user->block_user;
                                    $cnt_unread = \App\Service\BlockService::getUnreadBlock($obj_user->id);
                                @endphp
                                <tr class="{{ $obj_block_user->exist_not_read == 0 ? 'unread' : '' }}">
                                    <td class="td-detail"><a class="title_orange" href="{{ route('admin.fraud_block.detail', ['user'=>$obj_user->id]) }}">詳細</a></td>
                                    <td>
                                        <div class="flex profile" data-id="{{ $obj_user->id }}">
                                            ...
                                            <div>
                                                ...
                                                <p class="">最終更新日：{{ \Carbon\Carbon::parse($obj_block_user->end_reported_at)->format('Y.n.j') }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="vertical-am"><div class="flex-space cnt_count">{{ $obj_block_user->cnt_block }}件</div></td>
                                </tr>
                            @endforeach
                        @else
                            <tr>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.profile').click(function() {
                ...
                location.href = "{{ route('admin.fraud_block.detail') }}" + "/" + user_id;
            });
            ...
            $('#chk-not-read').change(function() {
                ...
                location.href="{{ route('admin.fraud_block.index') }}?chk_not_read="+chk_not_read;
            });

            $('#sort_type').change(function() {
                let order_type = $(this).val();
                location.href="{{ route('admin.fraud_block.index') }}?order="+order_type;
            });
        });
    </script>
@endsection
