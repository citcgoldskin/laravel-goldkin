        ...
        <section>
            <div class="lesson_info_area">
                ...
                <ul class="teacher_info_02 mt0">
                    <li class="icon">
                        <img src=" {{ $user_info['user_avatar'] }} " class="プロフィールアイコン">
                    </li>
                    <li class="about_teacher">
                        <div class="profile_name"><p>{{  $user_info['name'] }} {{ isset($menu_type) && $menu_type == config('const.menu_type.kouhai') ? 'センパイ' : 'コウハイ' }}<span>（{{ $user_info['age']}}）{{ $user_info['sex']}}</span></p></div>
        ...
        <section id="button-area">
            ...
            <div class="button-area">
                <div class="btn_base btn_pale-gray shadow-glay">
                    <button type="button" onclick="location.href='{{ route('user.myaccount.cancel_lesson',['schedule_id'=> $sch_id]) }}'">
                        このレッスンをキャンセルする
                    </button>
                </div>
            </div>
