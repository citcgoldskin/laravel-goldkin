            ...
            <div class="inner_box pb0">
                ...
                @foreach( $eval_types as $k => $v)
                    ...
                    <div class="check-flex">
                        <input type="checkbox" name="service_eval" id="{{$v['et_id']}}" value="" hidden>
        ...
        <div id="modal-hyouka" class="modal-content ok">
            ...
            <div class="button-area">
                <div class="btn_base btn_ok">
                    <a href="{{route('user.talkroom.talkData', ['menu_type'=>config('const.menu_type.kouhai'), 'room_id'=>$room_id])}}">OK</a>
                </div>
            </div>
        </div>
    </div>
...
@section('page_js')
    <script>
        ...
        $(".evalSendBtn").click(function () {
            ...
            var serviceEvals = $("input[name=service_eval]");
            var types = {};
            let all_marked = true;
            for (var i = 0, l = serviceEvals.length; l > i; i++) {
                types[serviceEvals[i].id] = serviceEvals[i].value;
                if (serviceEvals[i].value == "" || serviceEvals[i].value == null) {
                    $('#warning_'+serviceEvals[i].id).removeClass('hide');
                    all_marked = false;
                }
            }
