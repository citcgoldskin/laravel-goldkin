        ...
        <section>
            ...
            <ul class="komaru_list">
                @foreach($classes as $key => $val)
                    <li class="shadow-glay"><a href="{{route('user.myaccount.ques_list').'/'.$val['qc_id']}}">{{$val['qc_name']}}</a></li>
                @endforeach
            </ul>
        </section>
        ...
        <section id="question">
            ...
            @foreach($questions as $key => $val)
                <div class="form_wrap icon_form type_arrow_right shadow-glay mb15">
                    <button type="button" onClick="location.href='{{route("user.myaccount.ques_detail", ['id'=>$val['que_id'], 'page_from'=>'category'])}}'" class="form_btn">{{$val['que_ask']}}</button>
                </div>
            @endforeach
        </section>
