    ...
    <div class="tabs info_wrap three_tab mt0">
        ...
        <div class="tab_content" id="tab-01_content">
            ...
            <section class="type_summary">
                ...
                <div class="board_box">
                    <a href="{{ route('user.myaccount.student_lesson_detail', ['schedule_id' => $value['lrs_id']]) }}">
                        <ul class="teacher_info_02 mt0 icon_top">
                            <li class="icon_s40"><img src="{{ \App\Service\CommonService::getUserAvatarUrl($value['lesson_request']['lesson']['senpai']['user_avatar']) }}" class="プロフィールアイコン"></li>
                            <li class="about_teacher">
                                <div class="profile_name">
                                    <p class="lesson_ttl one_line">{{ $value['lesson_request']['lesson']['lesson_title'] }}</p>
                                    <p>{{ $value['lesson_request']['lesson']['senpai']['name'] }}
                                        <span>
                                            （{{ \App\Service\CommonService::getAge($value['lesson_request']['lesson']['senpai']['user_birthday']) }}）
                                            {{ \App\Service\CommonService::getSexStr($value['lesson_request']['lesson']['senpai']['user_sex']) }}
                                        </span>
                                    </p>
                                    <p class="target_area">レッスン日時：{{date('Y/n/j', strtotime($value['lrs_date'])) . '/' . \App\Service\CommonService::getHM($value['lrs_start_time']) }}~{{ \App\Service\CommonService::getHM($value['lrs_end_time']) }}</p>
                                </div>
                            </li>
                        </ul>
                    </a>
                        ...
                        <ul class="conditions_box pr0 pb30">
                            ...
                            <li>
                                <div class="form_wrap icon_form type_arrow_bottom shadow-glay">
                                    ...
                                    @endphp
                                    <select id="order_complete" name="order_complete" class="sort">
                                        @foreach($order_arr as $key => $value)
                                            @if(isset($order[config('const.schedule_state.complete')]) && $order[config('const.schedule_state.complete')] == $key)
                                                <option	selected='selected' value="{{$key}}">{{$value}}</option>
                                            @else
                                                <option  value="{{$key}}">{{$value}}</option>
                                            @endif
...
@section('page_js')
    <script>
        $(document).ready(function () {
            ...
            @if ( $order[config('const.schedule_state.complete')] == 4 )
            $('#date_comp').slideDown(500);
            @else
            $('#date_comp').slideUp(500);
            @endif

            @if ( $order[config('const.schedule_state.cancel_senpai')] == 4 )
            $('#date_can').slideDown(500);
            @else
            $('#date_can').slideUp(500);
            @endif
        });
        ...
        $('#order_complete').change(function () {
            var now = new Date();
            var from, to;
            if ( $(this).val() == 0 ) { // 今月のレッスン
                from = new Date(now.getFullYear(), now.getMonth(), 1);
                to  = new Date(now.getFullYear(), now.getMonth() + 1, 0)
                $('#from_complete').val(from.getFullYear() + '/' + (from.getMonth() + 1) + '/' + from.getDate());
                $('#to_complete').val(to.getFullYear() + '/' + (to.getMonth() + 1) + '/' + to.getDate());
        ...
        function filterList(state) {
            ...
            location.href = '{{ route('user.myaccount.student_lesson_history') }}' +
                '?state=' + state +
