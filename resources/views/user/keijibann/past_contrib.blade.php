...
@section('content')
    ...
    <div id="contents">
        ...
        <section class="tab_area tab_white mb0 pb10">
        ...
        <div class="tabs info_wrap three_tab nomt">
            <input id="tab-01" type="radio" name="tab_item"onclick="location.href='{{route("keijibann.recruiting")}}/1'">
            ...
            <input id="tab-03" type="radio" name="tab_item" checked="checked" onclick="location.href='{{route("keijibann.past_contrib")}}/1'">
            ...
            <div class="tab_content" id="tab_content">
                ...
                <section>
                    ...
                    <div class="board_box">
                        <a href="{{route('keijibann.recruiting_content') . '/' . $val['rc_id']}}">
                            ...
                            <div class="about_detail">
                                <p class="money">{{$val['rc_wish_minmoney']}}円～{{$val['rc_wish_maxmoney']}}円<small>{{$val['lesson_time']}}</small></p>
                                <p class="date_time">
                                    <span>{{$val['date']}}</span>
                        ...
                        <span class="end">{{ $val['rc_stop'] == config('const.lesson_stop_code.stop_lesson') ? "公開停止" : "終了" }}</span>
                    </div>
                    @endforeach
    ...
    <script>
        ...
        function getMoreRecruits()
        {
            ...
            $.ajax({
                type: "get",
                url: "{{route('keijibann.past_contrib')}}" + "/2" + "?page=" + pageCurrent,
                dataType: 'json',
                contentType: false,
                processData : false,
        ...
        function appendRecruits(data)
        {
            ...
            for(var key in data)
            {
                ...
                html += "<a href='" + "{{route('keijibann.recruiting_content')}}/" + recruit.rc_id + "'>";
                ...
                html += "</div></a>";
                html += "<span class='end'>終了</span>";
                html += "</div>";
            }
            $('#tab_content section').append(html);
            isLoading = false;
        }
